...
# 4 inputs/states：position, velocity, etc.
...
def get_cart_location(screen_width: int):
...
# %%
# 3. Training
# 3.1 Hyperparameters and utilities
BATCH_SIZE = 128
GAMMA = 0.999
EPS_START = 0.9
EPS_END = 0.05
...
def select_action(state: Tensor):
    global steps_done
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            action = policy_net(state).argmax() % n_actions
    else:
